...
### K 線頁疊加層
- **巴菲特指標** — 年度近似值，於 K 線右側多畫一條 % 軸（資料：`data/buffett.json`）
- **天體層** — 2020–2030 水/金/火/木/土 + 北交點轉星座事件，可選行星 + 切換箭頭/標籤/順逆行
  - 重新生成：`python scripts/gen_celestial.py` → 輸出 `data/celestial.json`
  - 演算法：NASA JPL `approx_pos` 軌道元素 + Kepler 解，地心黃經
  - 精度：水–火 ±1 天、木/土 ±3 天、北交點（mean node）<1 天
